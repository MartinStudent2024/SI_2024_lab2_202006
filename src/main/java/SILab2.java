...
public class SILab2 {
    public static boolean checkCart(List<Item> allItems, int payment){ // 1
        ...
        for(int i = 0; i < allItems.size(); i++){  // 5 (5.1 - init ; 5.2 - condition check; 5.3 - increment counter)
            ...
            if (item.getBarcode() != null){  // 9
                ...
                for (int j = 0; j < item.getBarcode().length(); j++){  // 12 (12.1 - init ; 12.2 - condition check; 12.3 - increment counter)
